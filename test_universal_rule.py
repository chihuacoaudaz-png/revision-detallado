...
def universal_std_turno(row, seq_in_day):
    ...
    # 1. SI GRUPO ES EXPLÍCITO (1 -> A, 2 -> B)
    if g_str in ("1", "1.0", "1,0"):
        return "A"
    if g_str in ("2", "2.0", "2,0"):
        return "B"
        
    # 2. SI TURNO ES EXPLÍCITO POR NÚMERO O LETRA COMÚN
    if raw_t in ("1", "1.0", "1,0", "A", "D", "DIA", "G1"):
        return "A"
    if raw_t in ("2", "2.0", "2,0", "B", "N", "NOCHE", "G2"):
        return "B"
        
    # 3. FALLBACK POR SECUENCIA EN EL DÍA
...
for idx, row in df_det.iterrows():
    ...
    t = universal_std_turno(row, seq_counter[key])
    std_turnos.append(t)
...
print(f"Total Discrepancias tras aplicar REGLA UNIVERSAL: {len(disc)}")
