...
def perfect_std_turno(row, seq_in_day):
    ...
    # 1. Especial AMERICANA: Turnos son letras B/C pero Grupos son 1.0/2.0
    if ctr == "AMERICANA":
    ...
    # 2. EVALUAR TURNO (COLUMNA H NATIVA DEL EXCEL)
    if raw_t in ("1", "1.0", "1,0", "A", "D", "DIA", "G1"):
        return "A"
    if raw_t in ("2", "2.0", "2,0", "B", "N", "NOCHE", "G2"):
        return "B"
        
    # 3. Si TURNO está vacío, evaluar GRUPO
    if raw_g in ("1", "1.0", "1,0"):
        return "A"
    if raw_g in ("2", "2.0", "2,0"):
        return "B"
        
    # 4. Fallback por orden de aparición en la fecha
...
for idx, row in df_det.iterrows():
    ...
    t = perfect_std_turno(row, seq_counter[key])
    std_turnos.append(t)
...
print(f"Total Discrepancias tras perfect_std_turno: {len(disc)}")
