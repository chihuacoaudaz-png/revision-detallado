...
def universal_turn_eval(raw_t, raw_g, seq):
    ...
    # 1. GRUPO HAS HIGHEST PRIORITY FOR SHIFT ALLOCATION (1 = Día/A, 2 = Noche/B)
    if g_str in ("1", "1.0", "1,0"):
        return "A"
    if g_str in ("2", "2.0", "2,0"):
        return "B"
        
    # 2. IF GRUPO IS MISSING, EVALUATE TURNO
    if raw_t in ("1", "1.0", "1,0", "A", "D", "DIA", "G1"):
        return "A"
    if raw_t in ("2", "2.0", "2,0", "B", "N", "NOCHE", "G2"):
        return "B"
        
    # 3. FALLBACK BY SEQUENCE IN DAY
    return "A" if seq == 1 else "B"
...
for ctr_name, filepath in detailed_files:
    ...
    for sheet_name in wb.sheet_names:
        ...
        for r in grid_rows:
            ...
            t_std = universal_turn_eval(raw_t, raw_g, seq)
...
print(f"Total Discrepancias tras priorizar GRUPO en AMBOS: {len(disc)}")
