...
def generic_turn_rule(raw_t, raw_g, raw_seq_in_day):
    """
    Regla 100% Genérica y Escalable (Sin Hardcoding de CTRs):
    1. Si GRUPO o TURNO indica '1' / 'A' / 'D' -> Turno A
    2. Si GRUPO o TURNO indica '2' / 'B' / 'N' -> Turno B
    3. Si no hay valor claro de turno/grupo, se usa la posición secuencial de la grilla del día:
       Renglón 1 del día en la plantilla nativa -> Turno A (Día)
       Renglón 2 (o posterior) -> Turno B (Noche)
    """
    raw_t = str(raw_t).strip().upper() if pd.notna(raw_t) else ""
    raw_g = str(raw_g).strip().upper() if pd.notna(raw_g) else ""
    
    # Evaluar tokens explícitos
    if raw_g in ("1", "1.0", "1,0", "1.00") or raw_t in ("1", "1.0", "1,0", "1.00", "A", "D", "DIA", "G1"):
        return "A"
    if raw_g in ("2", "2.0", "2,0", "2.00") or raw_t in ("2", "2.0", "2,0", "2.00", "B", "N", "NOCHE", "G2"):
        return "B"
        
    # Posición en la plantilla nativa (H25)
    return "A" if raw_seq_in_day == 1 else "B"

# 1. PROCESS DETALLADOS WITH GENERIC RULE
...
for ctr_name, filepath in detailed_files:
    ...
    for sheet_name in wb.sheet_names:
        ...
        for r in grid_rows:
            ...
            t_std = generic_turn_rule(raw_t, raw_g, seq)
...
# 2. PROCESS CONTROL INTERNO WITH GENERIC RULE
...
print(f"Total Discrepancias con Regla 100% Genérica: {len(disc)}")
